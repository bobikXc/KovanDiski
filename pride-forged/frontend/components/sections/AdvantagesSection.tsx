...
export function AdvantagesSection() {
  return (
    <section className="advantages-section relative isolate overflow-hidden px-4 py-14 sm:px-6 sm:py-16 lg:px-8 lg:py-14 xl:py-16">
      ...
      <div className="relative z-[2] mx-auto max-w-[1320px]">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:gap-4 xl:grid-cols-3">
          {advantages.map(([title, text], index) => (
            <Reveal key={title} className="h-full" delay={index * 0.08} amount={0.12}>
              <LiquidCard interactive className="advantages-card group h-full rounded-[22px] p-5 sm:p-6 lg:min-h-[185px] lg:rounded-[22px] lg:p-6">
                <div className="mb-3.5 flex h-8 w-8 items-center justify-center rounded-full border border-primary/15 bg-surface/65 text-[11px] font-black text-accent shadow-liquid">
                  {String(index + 1).padStart(2, "0")}
                </div>
                <h3 className="why-card-title text-lg font-bold leading-[1.2] tracking-normal text-primary lg:text-[1.08rem]">{title}</h3>
                <p className="why-card-text advantages-card-description mt-2 text-sm leading-[1.45] lg:text-[0.875rem]">{text}</p>
              </LiquidCard>
            </Reveal>
          ))}
        </div>
        <Reveal className="mx-auto mt-7 max-w-[980px] sm:mt-8" delay={0.14} amount={0.12}>
          <LiquidCard className="relative z-[2] rounded-[28px] border border-accent/25 px-5 py-6 text-left shadow-[0_24px_68px_rgb(var(--accent-rgb)/0.12)] sm:rounded-[32px] sm:px-8 sm:py-8 lg:px-9 lg:py-8 lg:text-center">
